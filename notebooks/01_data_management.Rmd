...
weather_data <- inner_join(forecasts_clean, cities_clean, by = c("State", "City")) %>%
  select(State, City, lon, lat, koppen, elevation, distance_to_coast, 
...
```{r combine-step3}
cat("Cleaned processed dataset dimensions:", dim(weather_data)[1], "rows by", dim(weather_data)[2], "columns\n")
cat("Original dataset dimensions:", dim(weather)[1], "rows by", dim(weather)[2], "columns\n")
```
...
- Peep values -- Integrity check (i.e. verify nothing super weird like 0xFF or 1.8683 (which would be super sus for what we expect))
```{r combine-step3}
